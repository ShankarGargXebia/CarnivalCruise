...
public class Actions {
	...
	try {
		...
		// when browser is not native
		    ...
		    dataContentFirst = "http://www.carnivalcorp.com/";
		...
		this.property.setStepDescription("checking broken links");
		verification = checkBrokenLinks();
		break;
		
	    case "checkbrokenimages":
		this.property.setStepDescription("checking broken images");
		verification = checkBrokenImages();
		break;
	    }
	...
	ArrayList<String> allLinksSet = new ArrayList<>();
	try {
	    Thread.sleep(5000);
	    String url = null;

	    List<WebElement> anchorTagsList = driver.findElements(byLocator(attributeType, attribute));
		...
		if (anchorTagElement != null) {
		    ...
		    boolean add = utility.check4DuplicateAndAddElement(allLinksSet, url);
		    if (add) {
			// System.out.println(url);
		    }
		}
	    ...
	    property.setRemarks("Below are the broken links: ");

	    for (int i = 0; i < allLinksSet.size(); i++) {
		String currentURL = allLinksSet.get(i);
		if (currentURL != null && !currentURL.contains("mailto")) {
			...
			if (statusCode != 200) {
			    ...
			    property.updateRemarks(currentURL);
			}
	    ...
	    System.out.println("Total no. of invalid links are "
	            + invalidLinksCount + " and they are:" + brokenLinkList);
	    if (invalidLinksCount == 0) {
		property.setRemarks("All the links on the page are valid.");
	    ...
	    property.setRemarks("While comparing the response thew an exception" + ":" + e.getMessage());
	    return false;
	}
	...
	List<String> brokenImageList = new ArrayList<String>();
	ArrayList<String> allImagesSet = new ArrayList<>();
	try {

	    String url = null;
	    // By cssImages =
	    // By.cssSelector(("div[style*='background-image']"));
	    // wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(cssImages));

	    explicitWait("elementsarevisible", null);
	    List<WebElement> imagesList = driver.findElements(byLocator(attributeType, attribute));
	    for (WebElement currentImageElement : imagesList) {

		if (currentImageElement != null) {
		    String elementStyle = null;
		    try {
			elementStyle = currentImageElement.getAttribute("style");
			System.out.println(elementStyle);
		    } catch (Exception e) {
			System.out.println("Exception : " + e);
		    }
		    if (elementStyle.contains("url")) {
			url = elementStyle.substring(elementStyle.indexOf("url(") + 5, elementStyle.indexOf("\")"));
		    } else {
		    }
		    boolean add = utility.check4DuplicateAndAddElement(allImagesSet, url);
		    if (add) {
			// System.out.println(url);
		    }
		}
	    }
	    // System.out.println("Total no. of links found are " +
	    // anchorTagsList.size());
	    // System.out.println("Total no. of Unique links found are " +
	    // (allLinksSet.size() - previuosSetSize));
	    // System.out.println("Getting Unique links for the current page
	    // completed.");
	    property.setRemarks("Below are the broken links: ");

	    for (int i = 0; i < allImagesSet.size(); i++) {
		String currentURL = allImagesSet.get(i);
		if (currentURL != null && !currentURL.contains("javascript")) {
		    if (currentURL.toLowerCase().contains("tronc.com")) {
			CloseableHttpClient client = HttpClientBuilder.create().build();
			HttpGet request = new HttpGet(currentURL);
			...
			if (statusCode != 200) {
			    System.out.println("Broken URL: " + currentURL);
			    invalidImagesCount++;
			    brokenImageList.add(currentURL);
			    property.updateRemarks(currentURL);
			}
		    }
		} else {
	    ...
	    System.out.println("Total no. of invalid links are "
	            + invalidImagesCount + " and they are:" + brokenImageList);
